...
import { UserDashboard } from "../shopper-4-icons/user-Dashboard";
            ...
            <BrowserRouter>
                ...
                <div ><h2 className="p-2 logo"  >Shopper<span style={{color:'#e6b800'}}>.</span></h2></div>
                    <nav className="header-button d-flex" >
                        ...
                        <div ><Link to="category/men's clothing" className='btn btnhead'>Men's Cloting</Link></div>
                        <div ><Link to="category/women's clothing" className='btn btnhead'>Women's Clothing</Link></div>
                    ...
                    <hr/>
                    <div className="icons">
                       <div> <Link to ="/search" ><span title="Search" className="bi bi-search me-2 iconhead"></span></Link></div>
                       <div className="me-2" ><NavLink to='/cart' className='me-2 iconhead'> <span title="Cart" className="bi bi-cart3 "  ><i style={{fontSize:'14px',float:'right',boxShadow:'1px'}}>{countstate}</i></span> </NavLink></div>
                        {/* <span title="Favourites" className="bi bi-heart me-2 iconhead"></span> */}
                        <div><span title="Account" onClick={handleClick} className="bi bi-person iconhead" ></span></div>
                        ...
                        >
                            <MenuItem onClick={handleClose}>Hello! -{cookie["UserId"]}</MenuItem>
                            <MenuItem onClick={handleClose} disabled={true}>My account</MenuItem>
                            <Link to='/login'><MenuItem onClick={SignOutClick}>Logout</MenuItem></Link>
                                    <MenuItem onClick={ThemeChange}  title='click to change the mode'><span className={icon} /> {mode }</MenuItem>

                        </Menu>
                        {/* <Link to='/dashboard' className="text-dark"><span className="bi bi-person me-2 iconhead"></span></Link> */}
                            </div>
